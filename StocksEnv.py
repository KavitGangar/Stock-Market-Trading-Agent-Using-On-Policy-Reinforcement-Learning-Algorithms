...
class StocksEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action[0] == 2:
            ...
            cur_value = self.portfolio_value()
            gain = cur_value - self.starting_portfolio_value
            retval = np.array(new_state),-ts_left+gain*10, False, { "msg": "nothing" }
        ...
        if action[0] == 0:
            if action[1] * apl_open[cur_timestep] > self.state[2]:
                ...
                retval = np.array(new_state), self.inaction_penalty-ts_left+(gain*10)+(partial*50), False, { "msg": "bought AAPL"}
        ...
        if action[0] == 3:
            ...
            if action[1] * msf_open[cur_timestep] > self.state[2]:
                ...
                self.buycount_2 = self.buycount_2 + 1
                ...
                retval = np.array(new_state), self.inaction_penalty-ts_left+(gain*10)+(partial*50), False, { "msg": "bought MSFT"}
        ...
        if action[0] == 1:
            ...
            if action[1] > self.state[0]:
                ...
                retval = np.array(new_state), -10000, False, { "msg": "sold more than have"}
            else:
                ...
                gain = cur_value - self.starting_portfolio_value
                partial = -(apl_close[cur_timestep] - apl_open[cur_timestep])
                retval = np.array(new_state), self.inaction_penalty-ts_left+(gain*10)+(partial*20), False, { "msg": "sold AAPL"}
        ...
        if action[0] == 4:
            ...
            if action[1] > self.state[1]:
                ...
                gain = cur_value - self.starting_portfolio_value
                partial = -(msf_close[cur_timestep] - msf_open[cur_timestep])
                retval = np.array(new_state), self.inaction_penalty-ts_left+(gain*10)+(partial*20), False, { "msg": "sold MSFT"}
    ...
    def reset(self):
        ...
        self.state[0] = 20 # random.randint(0,20)
        self.state[1] = 20 #random.randint(0,20)
        self.state[2] = 1000 #random.randint(100,2000)
        self.state[3] = apl_open[self.cur_timestep]
        self.state[4] = msf_open[self.cur_timestep]
