...
class StocksEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action[0] == 1:
            if action[1] > self.state[0]:
                ...
                retval = np.array(new_state), -ts_left +gain + (profit_sell*100) , False, { "msg": "sold AAPL"}
        ...
        if action[0] == 2:
            ...
            retval = np.array(new_state),  -self.inaction_penalty-ts_left +gain - 5 , False, { "msg": "nothing" }
        ...
        if action[0] == 0:
            if action[1] * apl_open[cur_timestep] > self.state[1]:
               ...
               # print("\nEpisode Terminating Bankrupt REWARD = " ,self.reward," - " ,self.buycount , " - " ,self.sellcount, "-" ,self.nothing ,"- ",self.nothingpseudo)
                ...
                gain = cur_value - self.starting_portfolio_value
                self.reward += -ts_left +gain
                retval = np.array(new_state), -ts_left + gain - cash_spent + 10  , False, { "msg": "bought AAPL"}
    ...
    def reset(self):
        ...
        self.state[0] = 10
        self.state[1] = 500 #random.randint(500,1000)
        self.state[2] = apl_open[self.cur_timestep]
        self.starting_portfolio_value = self.portfolio_value_states()
